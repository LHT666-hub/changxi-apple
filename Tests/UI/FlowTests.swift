...
final class FlowTests: XCTestCase {
    ...
    func testCoreJourneyAndScreenshots() {
        ...
        capture("03-health")
        let bmi = app.buttons["metric-BMI"]
        XCTAssertTrue(bmi.waitForExistence(timeout: 5))
        bmi.tap()
        XCTAssertTrue(app.staticTexts["中国成人 BMI 分级"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.staticTexts["BMI < 18.5"].exists)
        XCTAssertTrue(app.staticTexts["18.5 ≤ BMI < 24.0"].exists)
        XCTAssertTrue(app.staticTexts["24.0 ≤ BMI < 28.0"].exists)
        XCTAssertTrue(app.staticTexts["BMI ≥ 28.0"].exists)
        capture("03b-bmi-detail")
    ...
    func testLargeTextAndLandscape() {
        ...
        app.buttons["健康"].firstMatch.tap()
        XCTAssertTrue(app.buttons["metric-BMI"].waitForExistence(timeout: 5))
