...
- 首页 / 健康 / 服务 / 我的四个主 Tab；iPhone / iPad 始终保留底部导航；沉浸式对话采用全屏呈现。
...
- 文字对话、回复加载/取消/重试、持久历史、清空历史；附件、文字、麦克风和发送集中在单行输入工具栏。
- 全局“常曦”入口识别当前健康、预约、咨询、报告、计划、用药和反馈表单；确认后回填原页面，血压等结构化数据执行严格格式校验。
- 农历日期、月相节律详情与月亮进度；医生新回复可直接进入消息详情。
...
月池动效采用原生 SwiftUI 完成：角色轻呼吸与倾斜使用系统动画，水纹由 Canvas 实时绘制，月相和状态转场使用 SwiftUI 动画 API。项目不依赖 Rive，也不需要 `.riv` 资产。
