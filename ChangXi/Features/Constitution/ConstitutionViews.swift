                ...
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    VStack(spacing: 10) {
                        ForEach(choices.indices, id: \.self) { choiceIndex in
                            let score = choices[choiceIndex].0
                            let label = choices[choiceIndex].1
                            Button {
            ...
            if let result = store.result {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    Card {
                        ...
                        ForEach(Array(result.ranked.enumerated()), id: \.offset) { _, item in
...
private struct ReviewPipeline: View {
    ...
    var body: some View {
        Card {
            ...
            ForEach(Array([ConstitutionReviewStatus.draft, .pending, .confirmed].enumerated()), id: \.offset) { index, item in
                HStack(spacing: 12) {
