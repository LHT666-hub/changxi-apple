...
private struct ShiyangBrowseView: View {
    var body: some View {
        ...
        .shiyangAssistantAccess(title: "食养图鉴")
    }
}
...
private struct ShiyangProfileSettingsView: View {
    ...
    var body: some View {
        ...
        .navigationBarTitleDisplayMode(.inline)
        .shiyangAssistantAccess(title: "我的食养档案")
    }
...
private struct ShiyangHomeView: View {
    ...
    private var personalizedRecommendationNote: String {
        let serverSummary = store.data.shiyangRecommendationSummary.trimmingCharacters(in: .whitespacesAndNewlines)
        if !serverSummary.isEmpty { return serverSummary }
...
private struct ShiyangPantryView: View {
    ...
    @State private var generationHint = ""
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 22) {
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 96), spacing: 10)], spacing: 10) {
                    ForEach(categoryIngredients) { ingredient in
                        Button {
                            ...
                            VStack(spacing: 8) {
                                ShiyangIngredientArtwork(ingredient: ingredient)
                                    .frame(width: 54, height: 54)
                                    .background(
                                        selected.contains(ingredient.id) ? Color.white.opacity(0.64) : SY.cream.opacity(0.64),
                            ...
                            .background {
                                if selected.contains(ingredient.id) {
                                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                                        .fill(LinearGradient(colors: [SY.cream, SY.apricot.opacity(0.34)], startPoint: .topLeading, endPoint: .bottomTrailing))
                                } else {
                                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                            ...
                            .overlay {
                                RoundedRectangle(cornerRadius: 18, style: .continuous)
                                    .strokeBorder(selected.contains(ingredient.id) ? SY.apricot.opacity(0.72) : color(for: ingredient.category).opacity(0.16), lineWidth: selected.contains(ingredient.id) ? 1.4 : 0.8)
                            }
                            .overlay(alignment: .topTrailing) {
                                if selected.contains(ingredient.id) {
                                    Image(systemName: "checkmark.circle.fill")
                                        .symbolRenderingMode(.palette)
                                        .foregroundStyle(.white, SY.apricot)
                                        .font(.title3)
                                        .padding(8)
                ...
                Button {
                    ...
                    HStack(spacing: 10) {
                        ...
                        Text(isGenerating ? "常曦正在搭配…" : "用这些食材现配菜谱")
                    }
                    .frame(maxWidth: .infinity)
                }
                .buttonStyle(ShiyangPrimaryButton())
        ...
        .navigationBarTitleDisplayMode(.inline)
        .shiyangAssistantAccess(title: "我的食材")
        .onAppear {
            guard !loaded else { return }
    ...
    private func generateRecommendations() async {
        guard !selected.isEmpty, !isGenerating else { return }
        isGenerating = true
        generationHint = ""

        let excluded = Set(store.data.shiyangExcludedIngredientIDs)
        let local = ShiyangRecommendationEngine.recommendations(
            pantry: selected,
            excluded: excluded,
            maxMinutes: store.data.shiyangAvailableMinutes,
            lowSalt: store.data.shiyangLowSalt,
        ...
        var selectedRecipeID = local.first?.recipe.id
        var summary = "已按现有食材、时间和安全限制完成搭配。"
        if AppConfiguration.useRemoteAPI {
            do {
                let response = try await ShiyangNutritionService().rank(
                    ...
                    mealContext: store.data.shiyangMealContext,
                    goal: store.data.shiyangGoal,
                    healthNote: store.data.shiyangUseHealthData ? store.data.shiyangHealthNote : "",
                    medicationNote: store.data.shiyangUseMedicationData ? store.data.shiyangMedicationNote : ""
                )
                let allowed = Set(local.map(\.recipe.id))
                selectedRecipeID = response.recipeIds.first(where: allowed.contains) ?? selectedRecipeID
                summary = response.summary
            } catch {
                generationHint = "玄同暂时未连上，已先用本地安全规则完成。"
            }
        }

        if let selectedRecipeID { store.data.shiyangSelectedRecipeID = selectedRecipeID }
        store.data.shiyangRecommendationSummary = summary
        isGenerating = false
        MoonHaptics.shared.play(success: true, enabled: store.data.haptics)
        dismiss()
    }
    ...
    private func color(for category: ShiyangIngredientCategory) -> Color {
        ...
        case .pantry: CX.blue
        case .fruitDairy: Color(.displayP3, red: 0.55, green: 0.38, blue: 0.72)
        case .nutCondiment: Color(.displayP3, red: 0.62, green: 0.42, blue: 0.24)
        }
    }
}
...
private struct ShiyangRecipeDetailView: View {
    ...
    var body: some View {
        ...
        .navigationTitle("菜谱")
        .navigationBarTitleDisplayMode(.inline)
        .shiyangAssistantAccess(title: recipe.title)
    }
...
private struct ShiyangCookingGuideView: View {
    ...
    var body: some View {
        ...
        .navigationTitle("跟着做")
        .navigationBarTitleDisplayMode(.inline)
        .shiyangAssistantAccess(title: "制作\(recipe.title)")
...
private struct ShiyangAssistantAccessModifier: ViewModifier {
    @Environment(AssistantCoordinator.self) private var assistant
    ...
    func body(content: Content) -> some View {
        content
            ...
            .onDisappear { assistant.unregister(id: contextID) }
    }
}

private extension View {
    func shiyangAssistantAccess(title: String = "这顿饭的食养安排") -> some View {
        modifier(ShiyangAssistantAccessModifier(title: title))
    }
}
