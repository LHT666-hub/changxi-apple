...
enum ShiyangRecommendationEngine {
    static func recommendations(
        ...
        ShiyangCatalog.recipes.compactMap { recipe in
            ...
            // 用户明确选过的主食偏好应当压过“少缺一两样食材”的微小分差，
            // 否则问卷看似被记录，推荐顺序却没有可感知变化。
            if staplePreference == "米饭", ingredientIDs.contains("rice") { score += 28 }
            if staplePreference == "面食", ingredientIDs.contains("noodles") { score += 28 }
...
private struct ShiyangNutritionCandidate: Encodable {
    let id: String
    let title: String
    let minutes: Int
    let ingredientIds: [String]
...
private struct ShiyangNutritionRequest: Encodable {
    ...
    let lowSalt: Bool
    let likesSpicy: Bool
    let staplePreference: String
    let mealContext: String
    let goal: String
    let healthNote: String
    let medicationNote: String
    let candidateRecipes: [ShiyangNutritionCandidate]
}

struct ShiyangNutritionResponse: Decodable, Sendable {
    let model: String
...
struct ShiyangNutritionService: Sendable {
    let api: APIClient
    ...
    func rank(
        localRecommendations: [ShiyangRecommendation],
        pantry: Set<String>,
        excluded: Set<String>,
        city: String,
        maxMinutes: Int,
        lowSalt: Bool,
        likesSpicy: Bool,
        staplePreference: String,
        mealContext: String,
        goal: String,
        healthNote: String,
        medicationNote: String
    ) async throws -> ShiyangNutritionResponse {
        let candidates = localRecommendations.prefix(10).map { recommendation in
            ShiyangNutritionCandidate(
                id: recommendation.recipe.id,
                title: recommendation.recipe.title,
                minutes: recommendation.recipe.minutes,
                ingredientIds: recommendation.recipe.ingredients.map(\.ingredientID),
                tags: recommendation.recipe.tags
            )
        }
        let body = ShiyangNutritionRequest(
            pantryIds: pantry.sorted(),
            excludedIds: excluded.sorted(),
            city: city,
        ...
        return try await api.post("nutrition/recommendations", body: body, prefix: .legacy)
    }
}
